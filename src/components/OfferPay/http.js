...
class HttpOfferPay {
  ...
  pay(key, offer, amount) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (new BigNumber(initialWel).comparedTo(new BigNumber(amount)) >= 0) {
        resolve();
      } else {
      ...
      .then(() => new Promise((resolve, reject) => {
        if (amount && amount > 0) {
          ...
          offerState.push(new BigNumber(initialWel).minus(new BigNumber(amount))); // new balance in nectar ambassador
